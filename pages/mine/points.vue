<template>
  <view class="page">
    <view class="content">
      ...
      <view class="tabs">
        <view class="tab on">积分流水</view>
        <view class="tab" @click="goInvite">继续邀请</view>
      </view>
      <view class="card">
        <view v-if="!data.ledger.length" class="empty">暂无积分流水</view>
        <view class="list-item" v-for="item in data.ledger" :key="item.transaction_no">
          <view class="avatar">{{ item.direction === 'income' ? '+' : '-' }}</view>
          <view class="list-main">
            <view class="list-title">{{ ledgerTitle(item) }}</view>
            <view class="sub">{{ formatTime(item.created_at) }} · 余额 {{ item.balance_after }}</view>
          </view>
          <view :class="item.direction === 'income' ? 'money' : 'expense'">{{ item.direction === 'income' ? '+' : '-' }}{{ item.amount }}</view>
        </view>
      </view>
...
const data = reactive({ points: 0, granted_count: 0, records: [], ledger: [] })
...
const ledgerTitle = item => item.remark || ({
  invite_reward: '邀请奖励',
  travel_exchange: '兑换旅行路线',
  travel_exchange_refund: '旅行兑换退款',
  admin_adjust: '平台调整',
  points_expired: '积分到期',
  legacy_opening: '历史积分期初余额',
}[item.biz_type] || '积分变动')
