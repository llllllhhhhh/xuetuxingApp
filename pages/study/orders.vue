    ...
    <view v-else-if="filteredOrders.length" class="order-list">
      <view v-for="order in filteredOrders" :key="`${order.source}-${order.id}`" class="order-item">
        ...
        <view class="order-actions">
          <view v-if="order.payment_status === COMMERCE_PAYMENT_STATUS.PAID" class="action-group">
            <view class="action-button secondary" @tap="contactMerchant(order)">联系商户</view>
            <view
              v-if="order.source === 'standard' && !order.refund_status"
              class="action-button refund"
              :class="{ disabled: refundBusy === order.id }"
              @tap="requestRefund(order)"
            >
              {{ refundBusy === order.id ? '提交中...' : '申请退款' }}
            </view>
          </view>
...
import { createStudyOrderSupportConversation, getMyStudyOrders, requestStandardOrderRefund } from '../../utils/api.js'
...
const displayStatus = order => {
  if (order.refund_status === 'requested') return { text: '退款申请待处理', tone: 'processing' }
...
const actionHint = order => {
  if (order.refund_status === 'requested') return '等待平台处理退款申请'
...
const requestRefund = order => {
  ...
  uni.showModal({
    title: '申请退款',
    content: `确认申请退款 ¥${orderAmount(order)}？提交后由平台审核并原路退回。`,
    confirmText: '提交申请',
    confirmColor: '#d85d4b',
    success: async result => {
      ...
      try {
        const saved = await requestStandardOrderRefund(order.order_no)
        const index = orders.value.findIndex(item => item.source === order.source && item.id === order.id)
        if (index >= 0) orders.value.splice(index, 1, { ...order, ...saved, source: order.source })
