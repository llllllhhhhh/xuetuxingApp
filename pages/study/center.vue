    ...
    <view class="profile safe-top">
      ...
      <view class="profile-grid">
        <view class="profile-stat">
          <text>目标考试</text>
          <b>{{ data.profile?.target_exam || '待设置' }}</b>
        </view>
        <view class="profile-stat">
          <text>当前阶段</text>
      ...
      <view :class="['checkin-button', { done: checked }]" @tap="checkIn">
        ...
        <view>
          ...
          <text>{{ checked ? '保持节奏，明天继续' : '点击即可完成今日签到' }}</text>
        </view>
      ...
      <view v-for="right in data.entitlements" :key="right.id" class="right-card">
        <view class="right-head">
          ...
          <view class="right-title">
            <text>{{ typeName(right.product_type) }}</text>
            <b>{{ right.product_name }}</b>
          </view>
        ...
        <view class="lesson-list">
          <view v-for="lesson in right.product?.contents || []" :key="lesson.id" class="lesson">
            ...
            <view class="lesson-info">
              <b>{{ lesson.title }}</b>
              <text>{{ lesson.summary || '了解服务内容' }}</text>
            </view>
...
const load = async () => {
  try {
    ...
    checked.value = !!data.value.profile?.checked_today
  } catch (error) {
...
const checkIn = async () => {
  ...
  try {
    ...
    data.value.profile.checked_today = true
